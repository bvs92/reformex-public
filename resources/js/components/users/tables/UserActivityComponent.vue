<template>
  <div>
        ...
        <div class="overflow-auto" v-if="getTotalUserActivities > perPage">
            <div class="mt-3">
                <b-pagination 
                v-model="currentPage" 
                pills 
                :total-rows="getTotalUserActivities" 
                :per-page="perPage"
...
export default {
    ...
    computed: {
        ...mapGetters('activity', ['getUserActivities', 'getTotalUserActivities']),

        getTheUserActivities: function(){
            return this.getUserActivities.map(item => {
    ...
    created: function(){
        ...
        this.$store.dispatch('activity/initUserActivities', this.user_id).finally(() => {
            this.isBusy = false;
        });
    }
