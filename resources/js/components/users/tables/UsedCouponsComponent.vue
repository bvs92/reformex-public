  ...
  <div>
        ...
        >
            ...
            <template #cell(activated_at)="data">
                <b class="text-info">{{ formatElementTimeMethod(data.item.activated_at) }}</b>
                <!-- <b class="text-info">{{ data.item.created_at }}</b> -->
            </template>
        ...
        <div class="overflow-auto" v-if="getTotalUserActivatedCoupons > perPage">
            <div class="mt-3">
                <b-pagination 
                v-model="currentPage" 
                pills 
                :total-rows="getTotalUserActivatedCoupons" 
                :per-page="perPage"
...
export default {
    name: "UsedCouponsComponent",

    data() {
      return {
        // pagination
        // rows: this.getTotalUserActivatedCoupons,
        perPage: 25,
        currentPage: 1,
        ...
        //   { key: 'email', sortable: true },
          ...
          { key: 'created_at', sortable: true, label: 'Creat' },
          { key: 'activated_at', sortable: true, label: 'Activat' },
    ...
    computed: {
        ...mapGetters('coupons', ['getUserActivatedCoupons', 'getTotalUserActivatedCoupons']),

        getUserUsedCoupons: function(){
            return this.getUserActivatedCoupons.map(item => {
                this.formatElementTimeMethod(item.created_at);
                this.formatElementTimeMethod(item.activated_at);
                return item;
            })
        },
    ...
    created: function(){
        ...
        this.$store.dispatch('coupons/initUserActivatedCoupons', this.user_id).finally(() => {
            this.isBusy = false;
        });
    }
