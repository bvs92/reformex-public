<template>
  ...
  <div class="row my-2" v-else>
        ...
        <b-modal v-model="modalShow" :title="'Detalii plata '" centered ok-only scrollable >
            <ShowPaymentDetails ref="showPaymentDetails" :payment_uuid="selectedPayment" />
        </b-modal>
    </div>
</template>
...
export default {
    ...
    data(){
        return {
            ...
            fields: [
                ...
                { key: 'user.email', sortable: true, label: 'E-mail' },
    ...
    async created(){
        this.isLoading = true;
        await this.$store.dispatch('payments/allPayments').finally(() => {
