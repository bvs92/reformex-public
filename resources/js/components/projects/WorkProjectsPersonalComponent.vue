<template>
  <div>
        ...
        <div class="overflow-auto" v-if="getTotalPersonalProjects > perPage">
            <div class="mt-3">
                <b-pagination 
                v-model="currentPage" 
                pills 
                :total-rows="getTotalPersonalProjects" 
                :per-page="perPage"
...
export default {
    name: "WorkProjectsPersonalComponent",

    data() {
      return {
        // pagination
        // rows: this.getTotalPersonalProjects,
        perPage: 20,
        currentPage: 1,
        ...
        fields: [
          ...
          { key: 'number_photos',label: 'Număr fotografii', sortable: true },
    ...
    computed: {
        ...mapGetters('projects', ['getPersonalProjects', 'getTotalPersonalProjects']),

        getTheCoupons: function(){
            return this.getPersonalProjects.map(item => {
    ...
    created: function(){
        ...
        this.$store.dispatch('projects/initPersonalProjects').finally(() => {
            this.isBusy = false;
        });
    }
