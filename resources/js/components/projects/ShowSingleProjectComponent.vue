...
<div class="grid-margin">
                    ...
                    <ValidationObserver ref="observer" v-slot="{ handleSubmit, invalid }" >
                        <form enctype="multipart/form-data" @submit.prevent="handleSubmit(onSubmit)">
                            <div class="form-group row my-6">
                                ...
                                <div class="col-sm-10" v-if="files && files.length > 0">
                                    <button type="submit" class="btn btn-success" v-if="!btnLoading">Adaugă imaginile în proiect</button>
                                    <button class="btn btn-success btn-loading" v-else>Se adaugă imaginile</button>
                                </div>
                            </div>
                        </form>
                    </ValidationObserver>
    ...
    <div v-else>
        <p class="text-center">Proiectul nu există.</p>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        onSubmit: function(){
            ...
            axios.post(`/api/work-projects/${this.project.uuid}/upload/photos`, formData, config).then(response => {

                if(response.data.result == true){
                    // if success => reseteaza cerere.
                    Vue.$toast.open({
                        message: 'Felicitări! Proiectul a fost actualizat!',
                        type: 'success',
                        duration: 6000
                    });
