        ...
        <div class="col-lg-12">
            ...
            >
                ...
                <template #cell(invoice)="row">
                    <a class="btn btn-sm btn-info text-white" @click.prevent="download(row.item)">
                    Descarcă
                    </a>
                </template>
            ...
            <div class="overflow-auto" v-if="getTotalPayments > perPage">
                <div class="mt-3">
                    <b-pagination 
                    v-model="currentPage" 
                    pills 
                    :total-rows="getTotalPayments" 
                    :per-page="perPage"
...
export default {
    ...
    computed: {
        ...mapGetters('payments', ['getPayments', 'getTotalPayments', 'getPayment']),
    ...
    data(){
        return {
            ...
            sortDesc: true,
            fields: [
                { key: 'uuid', sortable: true, label: 'ID plată' },
                { key: 'user.email', sortable: true, label: 'E-mail utilizator' },
                { key: 'created_at', sortable: true, label: 'Dată plată' },
                { key: 'payment_status', sortable: true, label: 'Status plată' },
                { key: 'invoice', sortable: true, label: 'Factură' },
    ...
    methods: {
        ...
        openPayment: async function(payment_uuid){
            this.modalShow = !this.modalShow;
            this.selectedPayment = payment_uuid;
            // this.$refs.showPaymentDetails.payment = payment_uuid
        }
    },

    async created(){
        this.isLoading = true;
        await this.$store.dispatch('payments/allPaymentsInvoices').finally(() => {
