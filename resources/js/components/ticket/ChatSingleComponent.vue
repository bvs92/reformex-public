...
<div v-else 
    class="d-flex" 
    :class="response.user_id == current_user.id ? 'justify-content-end' : 'justify-content-start'" 
    @mouseover="show_options = true" 
    @mouseleave="show_options = false"
    >

    <div v-if="response.user_id == current_user.id">
    ...
    <template v-if="response.user_id == current_user.id">
        ...
        <div class="w-auto msg_cotainer_send" style="min-width: 20%; max-width: 80%;">
            <p style="text-align: right;">{{ response.message }}</p>
            
            <div class="mt-2 row" v-if="response.attached_files && response.attached_files.length > 0">
            ...
            <span class="msg_time_send">{{ this.calculateResponseTime }}</span>

        </div>


        <div class="img_cont_msg" v-if="response.user_details">
            <img :src="'/' + response.user_details.profile_photo" class="rounded-circle user_img_msg" alt="img">
            <!-- <img src="/assets/images/users/10.jpg" class="rounded-circle user_img_msg" alt="img"> -->
        </div>
        <div class="img_cont_msg" v-else>
            <img src="/images/avatars/default-photo.png" class="rounded-circle user_img_msg" alt="img">
            <!-- <img src="/assets/images/users/10.jpg" class="rounded-circle user_img_msg" alt="img"> -->
        </div>
    </template>
    ...
    <template v-else>
        ...
        <div class="msg_cotainer w-auto" style="min-width: 20%; max-width: 80%;">
            {{ response.message }}
            <span class="msg_time">{{ this.calculateResponseTime }}</span>
            <div class="row mt-2" v-if="response.attached_files && response.attached_files.length > 0">
                <FileComponent :attached_files="response.attached_files" :content_align="response.user_id == current_user.id ? 'justify-content-end' : 'justify-content-start'" />
            </div>
