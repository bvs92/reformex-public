...
<div class="col-lg-12 my-2"> 
    ...
    <div class="card border-dark mb-3">
        ...
        <div class="card-body text-dark">
            ...
            <div v-if="banner.ends_at" class="card-text">
            ...
            <div v-if="extend">
                ...
                <form class="col-lg-12" @submit.prevent="handleSubmit(updatePeriod)">
                    ...
                    <div class="d-flex justify-content-around">
                        <button type="submit" class="btn btn-success my-2" v-if="!loading_period">Salvează</button>
                        <b-button variant="primary" disabled v-else>
                            <b-spinner small type="grow"></b-spinner>
                            Salvez...
                        </b-button>
                        <button class="btn btn-default my-2" @click.prevent="extend = !extend">Înapoi</button>
                    </div>
                </form>
                </ValidationObserver>
            </div>
            <div v-else>
                <button class="btn btn-primary btn-sm my-2" @click.prevent="extendValability">Extinde perioadă</button>
            </div>
        ...
        <div class="card border-dark mb-3">
            <div class="card-header">Categorii</div>
                <div class="card-body text-dark">
                    <div v-if="banner.categories && !change_categories">
                        <p>Categorii curente</p>
                    <div>
...
export default {
    ...
    methods: {
        ...
        updateOptions: async function(){
            ...
            await axios.post('/api/banners/update_announce_options/' + this.banner.id, formData)
            .then(async response => {
        ...
        updatePeriod: async function(){
            ...
            this.loading_period = true;
            let formData = new FormData();
            formData.append('period',  this.period);
          
            await axios.post('/api/banners/update_announce_period/' + this.banner.id, formData)
            .then(async response => {
                ...
                if(response.data.success){
                    ...
                    this.recent_period = response.data.recent_period;

                    
          
                } else if(response.data.validation_errors){
                    Vue.$toast.open({
                        message: 'Oups! Am întâmpinat erori de validare. Verifică opțiunea aleasă.',
                        type: 'error',
                        duration: 6000,
                        position: 'bottom'
                    });
        ...
        updateCategories: async function(){
            ...
            await axios.post('/api/banners/update_announce_categories/' + this.banner.id, formData)
            .then(async response => {
                console.log(response.data);
                if(response.data.success){
                    // adauga user in lista.
    ...
    created(){
        this.ends_at = this.banner.ends_at;
        this.recent_period = this.banner.recent_period;
        this.hasForm = this.banner.has_form == 1 ? true : false;
        this.show_email = this.banner.show_email == 1 ? true : false;
        this.status = this.banner.status == 1 ? true : false;

        if(this.banner.categories){
            this.listing_categories = this.banner.categories;
            this.getExistingCategories(this.banner.categories);
        }
    }
}
</script>