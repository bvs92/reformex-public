<template>
  <div class="col-lg-12" v-if="banner.processing == 1">
        <div class="card border-primary mb-3">
            ...
            <div class="card-body text-primary d-flex justify-content-around">
                <a href="#" @click.prevent="activateBanner" class="btn btn-success" v-if="!activateStatus">Activează</a>
        ...
        <b-modal v-model="rejectModal" hide-footer title="Respinge banner">
            <b-form-textarea
                id="textarea"
                v-model="message"
            ...
            <a href="#" @click.prevent="rejectBanner" class="btn btn-danger mx-auto d-block my-2" v-if="!rejectStatus">Respinge</a>
...
export default {
    ...
    methods: {
        activateBanner: async function(){
            this.activateStatus = true;

            await axios
                .post('/api/banners/activate/' + this.banner.uuid)
                .then(async response => {
                    if(response.data.success){
                        Vue.$toast.open({
                            message: 'Banner activat cu succes.',
                            type: 'success',
                            duration: 6000,
                            position: 'bottom'
                        });
        
                        this.$emit('activate_banner', true);
        ...
        rejectBanner: async function(){
            ...
            await axios
                .post('/api/banners/reject/' + this.banner.uuid, formData)
                .then(async response => {
                    if(response.data.success){
                        Vue.$toast.open({
                            message: 'Banner respins cu succes.',
                            type: 'success',
                            duration: 6000,
                            position: 'bottom'
                        });
