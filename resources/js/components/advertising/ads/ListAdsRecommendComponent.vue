...
<div class="">
        ...
        <div class="row mb-5 p-5" style="background: lightgrey;">
            ...
            <div class="col-lg-6 d-flex justify-content-center">
                <button class="btn btn-info mx-auto" @click.prevent="loadAds">Preluare anunțuri</button>
            </div>
        </div>
        <br><br>
        <div class="row my-2">
        ...
        >
            ...
            <template #cell(actions)="row">
                <a class="btn btn-sm btn-info" :href="'/publicitate/admin/anunturi-recomandate/detalii/' + row.item.uuid">
                Vezi
                </a>
            </template>
        ...
        <div class="overflow-auto" v-if="getTotalAds > perPage">
            <div class="mt-3">
                <b-pagination 
                v-model="currentPage" 
                pills 
                :total-rows="getTotalAds" 
                :per-page="perPage"
...
export default {
    name: "ListAdsRecommendComponent",
        ...
        loadAds: async function(){
            if(this.selected_type !== null){
                ...
                if(this.selected_type == 'processing'){
                    this.isBusy = true;
                    await this.$store.dispatch('ads_recommend/loadProcessingAds').finally(() => {
                        this.isBusy = false;
                    });
                } else  {
                    this.isBusy = true;
                    await this.$store.dispatch('ads_recommend/loadAds', this.selected_type).finally(() => {
                        this.isBusy = false;
                    });
                }
