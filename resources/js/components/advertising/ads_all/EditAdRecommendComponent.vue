<template>
<div>
    <div class="row justify-content-center" v-if="getAd.paid == 0">
        <div class="card col-lg-8">
            <div class="card-header">
                <h3 class="card-title">Anunțul este inactiv</h3>
                <div class="card-options">
                    <a href="#" class="card-options-collapse" data-toggle="card-collapse"><i class="fe fe-chevron-up"></i></a>
                </div>
            </div>
            <div class="card-body">
                <p>Pentru a activa anunțul este necesară efectuarea plății.</p>
                <br>
                <a :href="`/publicitate/anunturi-recomandate/detalii/${uuid}/activare`" class="btn btn-primary">Activare anunț</a>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-lg-12 my-3">
            <div class="d-flex justify-content-end" v-if="getAd.rejected == 0">
                <button class="btn btn-sm btn-danger mr-3" @click.prevent="deleteAd" v-if="!deleteStatus">Elimină</button>
    ...
    <ValidationObserver ref="observer_announce" v-slot="{ handleSubmit, invalid }">
        <form class="row mt-5" @submit.prevent="handleSubmit(updateAnnounce)">
            ...
            <div class="col-lg-6 col-sm-12 my-2">
                <validation-provider rules="" v-slot="{ errors, invalid, passed, touched }">
                    <label for="website">Site / URL extern</label>
                    <input type="text" class="form-control" 
                    :class="{'is-invalid' : touched && invalid, 'is-valid': passed}" 
                    id="website" 
...
export default {
    ...
    methods: {
        ...
        updateAnnounce: async function(){
            ...
            await axios.post('/api/ads_recommend/personal/update_announce/' + this.getAd.id, formData).then(async response => {
        ...
        deleteAd: async function(){
            ...
            this.$swal({
                ...
                if (result.isConfirmed) {
                    await axios.delete('/api/ads_recommend/personal/delete/' + this.uuid)
                        .then(async response => {
                            if(response.data.success){
                                Vue.$toast.open({
                                ...
                                location.replace('/publicitate/anunturi-recomandate')
