...
<div class="text-center" v-if="loading">
  ...
  <div v-else>
      <div>
        <b-table striped hover :items="reported_reviews" :fields="fields" :busy.sync="isBusy" show-empty>
            <template #table-busy>
...
export default {
    name: "ListReportedReviewsAdminComponent",
    ...
    computed: {
        ...mapGetters('reviews', ['getReportedReviews', 'getCurrentPage', 'getFrom', 'getLastPage', 'getPerPage', 'getTotal',]),
    },
    ...
    methods: {
        ...
        pageClicked: async function(page){
            this.isBusy = true;
            await this.$store.dispatch('reviews/getReportedReviewsFromPage', page).then(() => {
                this.reported_reviews = this.getReportedReviews;
            }).finally(() => {
                this.isBusy = false;
            });
        ...
        deleteReview: async function(id){
            ...
            await this.$store.dispatch('reviews/deleteReportedReview', id).then(() => {
                this.reported_reviews = this.getReportedReviews;
    ...
    created(){
        // this.loading = true;
        this.isBusy = true;
        this.$store.dispatch('reviews/getReportedReviewsFromPage', this.currentPage).then(() => {
            console.log('getReportedReviewsFromPage', this.getReportedReviews);
            this.reported_reviews = this.getReportedReviews;
        }).finally(() => {
            // this.loading = false;
            this.isBusy = false;
