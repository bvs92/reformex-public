...
export default {
    name: "ListAllReviewsAdminComponent",
    ...
    computed: {
        ...mapGetters('reviews', ['getAllReviews', 'getCurrentPage', 'getFrom', 'getLastPage', 'getPerPage', 'getTotal',]),
    },
    ...
    methods: {
        ...
        pageClicked: async function(page){
            this.isBusy = true;
            await this.$store.dispatch('reviews/getAllReviewsFromPage', page).then(() => {
                this.all_reviews = this.getAllReviews;
            }).finally(() => {
                this.isBusy = false;
            });
        ...
        deleteReview: async function(id){
            ...
            await this.$store.dispatch('reviews/deleteReview', id).then(() => {
                this.all_reviews = this.getAllReviews;
    ...
    created(){
        // this.loading = true;
        this.isBusy = true;
        this.$store.dispatch('reviews/getAllReviewsFromPage', this.currentPage).then(() => {
            this.all_reviews = this.getAllReviews;
        }).finally(() => {
            // this.loading = false;
            this.isBusy = false;
