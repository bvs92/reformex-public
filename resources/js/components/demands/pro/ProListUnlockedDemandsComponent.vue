<template>
   <div class="mt-4">
        ...
        <div class="overflow-auto" v-if="getTotalUnlockedDemands > perPage">
            <div class="mt-3">
                <b-pagination 
                v-model="currentPage" 
                pills 
                :total-rows="getTotalUnlockedDemands" 
                :per-page="perPage"
...
export default {
    name: "ProListUnlockedDemandsComponent",
    ...
    data() {
      return {
        ...
        fields: [
          ...
          { key: 'city', sortable: true, label: 'Oraș' },
    ...
    computed: {
        ...mapGetters('pro_demands', ['getUnlockedDemands', 'getTotalUnlockedDemands']),

        getTheDemands: function(){
            if(this.getTotalUnlockedDemands > 0){
                return this.getUnlockedDemands.map(item => {
    ...
    methods: {
        ...
        goToPage: function(_item){
            ...
            window.location = '/cereri/pro/detalii/' + _item.item.uuid;
        },
    ...
    created: function(){
        ...
        this.$store.dispatch('pro_demands/initUnlockedDemands').finally(() => {
            this.isBusy = false;
        });
    }
