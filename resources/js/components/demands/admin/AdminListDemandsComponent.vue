...
export default {
    name: "AdminListDemandsComponent",
    ...
    computed: {
        ...mapGetters('admin_demands', ['getDemands', 'getTotalDemands']),

        getTheDemands: function(){
            if(this.getTotalDemands > 0){
                return this.getDemands.map(item => {
    ...
    created: function(){
        ...
        this.$store.dispatch('admin_demands/initDemands').finally(() => {
            this.isBusy = false;
        });
    }
