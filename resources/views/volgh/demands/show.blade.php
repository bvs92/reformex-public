...
{{--                                         
                                    ...
                                    <div class="col-md-4 p-4">
                                        ...
                                        @if($demand->hasProfessional(auth()->user()->professional))
                                            ...
                                            <div class="row">
                                                <div class="col-lg-6">
                                                    <a href="tel:{{$demand->phone}}" rel="nofollow" class="btn btn-success btn-block"><i class="fa fa-phone"></i> Apeleaza</a>
                                                </div>
                                                <div class="col-lg-6">
                                                    @if($timeline)
                                                        @if($timeline->hasUUID())
                                                            <a href="{{ route('timeline.show.pro.uuid', $timeline->uuid) }}" class="btn btn-primary btn-block" data-toggle="tooltip" data-placement="top" title="#{{ $timeline->uuid }}"><i class="fa fa-send"></i> Conversatie</a>
                                                        @else
                                                            <a href="{{ route('timeline.show.pro', $timeline->id) }}" class="btn btn-primary btn-block"><i class="fa fa-send"></i> Conversatie #{{ $timeline->id }}</a>
                                                        @endif
                                                    @endif
                                                </div>
                                            </div>
                                        @else
                                            <!-- daca demand nu-mi apartine, arata buton de cumparare -->
                                            @if(!$demand->belongsToMe())

                                                @if(!$demand->hasUUID())
                                                <form action="{{ route('demands.buy', $demand->id) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="btn btn-primary btn-block my-2"><i class="side-menu__icon ti-unlock"></i> Deblocheaza cererea ({{ $demand->getCalculatedPriceNormal() }} RON)</button>
                                                </form>
                                                @else
                                                <form action="{{ route('demands.buy.uuid', $demand->uuid) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="btn btn-primary btn-block my-2"><i class="side-menu__icon ti-unlock"></i> Deblocheaza cererea ({{ $demand->getCalculatedPriceNormal() }} RON)</button>
                                                </form>
                                                @endif
