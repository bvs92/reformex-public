    ...
    <div class="list d-flex align-items-center border-bottom p-2">
        ...
        <div class="wrapper w-100 ml-3" style="margin:10px auto;">
            ...
            <div class="row mt-1">
                ...
                <div class="col-lg-10">
                    <p class="mt-3">
                        {{ \App\User::find($notification->data['user_id'])->getName() }} (@if(\App\User::find($notification->data['user_id'])->hasRoles()) {{ ucfirst(\App\User::find($notification->data['user_id'])->getFirstRole()->name) }} @endif)
                    </p>
                </div>
            </div><!-- end row -->
            ...
            <p class="mb-0 d-flex">
                <b><strong>{{ \App\User::find($notification->data['user_id'])->getName() }}</strong> a raspuns la raportarea cererii (#{{ $notification->data['demand_id'] }})</b>
            </p>
