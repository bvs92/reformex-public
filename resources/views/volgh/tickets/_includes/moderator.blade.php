                                    ...
                                    @if($tickets && $tickets->count() > 0)
                                        @foreach($tickets as $ticket)
                                            <tr>
                                                ...
                                                <td>
                                                    {{-- {{ $ticket->resolvers()->count() }} @if($ticket->resolverIsMe())<span>(eu)</span>@endif --}}
                                                    @if($ticket->resolvers()->count() > 0)
                                                        @foreach($ticket->resolvers as $resolver) 
                                                        <p class="text-small">{{ $resolver->user->getName() }} @if($resolver->user->id == auth()->user()->id)<span>(eu)</span>@endif</p> 
                                                        @endforeach
                                                    @else
                                                    ...
                                                    <div class="row">
                                                        <div class="dropdown float-right">
                                                            ...
                                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                                                ...
                                                                <a onclick="event.preventDefault();deleteTicket('{{$ticket->uuid}}')" class="dropdown-item"><i class="ti-trash"></i> Elimina</a>
                                                                {{-- <a onclick="event.preventDefault();document.getElementById('deleteTicket-{{$ticket->uuid}}').submit();" class="dropdown-item"><i class="ti-trash"></i> Elimina</a> --}}
                                                                <form 
                                                                    action="{{ route('tickets.destroy.uuid', $ticket->uuid) }}" 
                                                                    id="deleteTicket-{{ $ticket->uuid }}" 
                                                                    method="POST" 
                                                                    style="display: none;">
                                                                    @csrf
