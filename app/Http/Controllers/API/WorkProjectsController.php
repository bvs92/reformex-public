...
class WorkProjectsController extends Controller
{
    ...
    public function getProjectPhotosById($id)
    {
        $project = WorkProject::find($id);
    ...
    public function store(Request $request)
    {
        ...
        $validated_project['uuid'] = $this->generateUUID();
        $validated_project['user_id'] = $user->id;
        ...
        if (!$new_project = WorkProject::create($validated_project)) {
            DB::rollBack();
            return response()->json(['errors' => true]);
        }
        ...
        if ($request->hasFile('the_files')) {
            ...
            foreach ($all_files as $one_file) {
                ...
                $wirkProjectPhoto->work_project_id = $new_project->id;
                $wirkProjectPhoto->user_id = $user->id;
        ...
        $new_project->categories()->sync(json_decode($final_categories));
    ...
    public function uploadPhotosAsAdmin(Request $request, $uuid)
    {
        try {
            $user = auth()->user();
        } catch (\Tymon\JWTAuth\Exceptions\UserNotDefinedException $e) {
    ...
    public function destroyAsAdmin($uuid)
    {
